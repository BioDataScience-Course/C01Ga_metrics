# Métriques et optimisation en classification supervisée

Ce projet nécessite d'avoir assimilé l'ensemble des notions relatives aux métriques du premier module du cours de Science des Données biologiques III pour sa première partie. Il nécessite ensuite d'avoir compris les concepts des modules 1 à 3 pour sa seconde partie. Il correspond au dépôt GitHub <https://github.com/BioDataScience-Course/C01Ga_metrique>.
...
Placez votre aide-mémoire à la racine de ce projet et nommez-le `metrics.pptx` ou `metrics.key` selon le format choisi. **Exportez aussi une version au format PDF que vous nommerez obligatoirement `metrics.pdf` dans le même dossier racine du projet.**
